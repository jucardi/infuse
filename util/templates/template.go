...
func (t *Template) Parse(writter io.Writer, data interface{}) error {
...
func (t *Template) LoadFileDefinitionsByPattern(pattern string) error {
	result, err := loadTemplates(pattern)
	if err != nil {
		return err
	}
...
func (t *Template) LoadFileDefinition(files ...string) error {
	for _, filename := range files {
		if tmplStr, err := loadTemplate(filename); err != nil {
			return err
